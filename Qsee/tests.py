...
class AssayTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # Form validation should prevent this from occuring
        Test.objects.create(
                result=50,
                test_date="14/01/2022",
                control_id=Control.objects.get(control_name="test_control"),
                analyser_id=Analyser.objects.get(analyser_name="test_analyser"),
                operator="test_operator",
                note="result is over 45 Ct")
    ...
    def test_database_add(self):
        ...
        self.assertEqual(len(test_tests), 21, "Check minium number of records have been entered for tests table")
    ...
    def test_duplicates(self):
        """Check to see if there are duplicate names for assays and analysers"""
        pass

    def test_check_date_format(self):
        """Check to see if the date has been entered in the corret format."""
        tests = Test.objects.all()
        self.assertEqual(tests[1].test_date, "2/01/2022") # correct
        # self.assertEqual(tests[1].test_date, "02/01/2022") # incorrect
